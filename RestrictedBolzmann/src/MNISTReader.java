...
public class MNISTReader extends JFrame {
	...
	static final double LEARN_RATE = 0.5;
	int numLabels;
	int numImages;
	int numRows;
	int numCols;	
	...
	public void activateForward(double in[], double w[][],double out[]){
		for (int i = 0; i < NEURONS; i++) {
			double sumOfHiddenInputs = 0;
			//Multiply each input by the weight corresponding to an output and sum it up
			for(int j = 0; j < NEURONS; j++) {
				sumOfHiddenInputs += in[j] * w[j][i];
			}
			//Apply sigmoid to sum of hidden node inputs and save as output
			out[i] = 1/(1 + Math.exp(-sumOfHiddenInputs));
		}
	}
	public void activateReconstruction(double rec[], double w[][],double out[]){
		//Init BIAS for reactivation
		out[0] = 1;
		for (int i = 0; i < NEURONS; i++) {
			double sumOfReconInputs = 0;
			//Multiply each output by the weight corresponding to an output and sum it up
			for(int j = 0; j < NEURONS; j++) {
				sumOfReconInputs += out[j] * w[i][j];
			}
			//Apply sigmoid to sum of hidden node inputs and save as output
			rec[i] = 1/(1 + Math.exp(-sumOfReconInputs));
		}
	}
	...
	public void contrastiveDivergence(double inp[], double out[], double rec[], double w[][])
	{
		for(int i = 0; i < NEURONS; i++) {
			for(int j = 0; j < NEURONS; j++) {
				w[j][i] += LEARN_RATE * (out[i]*inp[j] - out[i] * rec[j]);
			}
		}
